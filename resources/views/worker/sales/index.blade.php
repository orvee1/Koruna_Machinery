...
<div class="container">
    ...
    {{-- 🔍 Filter --}}
    <form method="GET" action="{{ route('worker.sales.index') }}" class="row gy-2 gx-3 align-items-end mb-4">
        <div class="col-auto">
            <label>Date</label>
            ...
            <tbody>
                @foreach($bills as $bill)
                    <tr>
                        ...
                        <a href="{{ route('worker.sales.show', $bill->id) }}" class="btn btn-sm btn-outline-info">View</a>
                        <form action="{{ route('worker.sales.destroy', $bill->id) }}" method="POST" class="d-inline-block" onsubmit="return confirm('Are you sure you want to delete this bill and all related sales?');">
                            @csrf
                            @method('DELETE')
